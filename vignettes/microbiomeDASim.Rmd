...
\vdots \\
Y_{nq_{n}}
\end{pmatrix}$$ with $Y_{ij}$ representing the $i^{th}$ patient at the $j^{th}$ 
...
Below we show how to generate data in the package `microbiomeDASim` 
...
We can now look at sample data generated for our trivial example,

```{r, fig.align='center', fig.height=4, fig.width=6}
#Trivial example described above
triv_ex <- mvrnorm_sim(n_control=10, n_treat=10, control_mean=2, sigma=1,
            num_timepoints=2, rho=0.8, corr_str="ar1",
            func_form="linear", beta= c(0, 1), missing_pct=0,
            missing_per_subject=0, dis_plot=TRUE)
head(triv_ex$df)
triv_ex$Sigma[seq_len(5), seq_len(5)]
```
...
+ L_up/L_down (Hockey Stick Trends)
...
hockey_sim <- mvrnorm_sim(n_control=10, n_treat=10, control_mean=2, sigma=1,
                            ...
                            func_form="L_up", beta= 0.5, IP=5, missing_pct=0,
                            missing_per_subject=0, dis_plot=TRUE)
```

The `mvrnorm_sim` method generates a single feature with a specified 
...
To address this we can use the function `gen_microbiome`, which specifies the 
number of features and the number of differentiall abundant features. All 
...
+ `bug_feat` identifies the subject ID, time, group (Control vs. Treatment) and 
corresponding Sample ID from the columns of `Y`.
