...
\author{Justin Williams, Hector Corrada Bravo, Jennifer Tom, Joseph Nathaniel
    Paulson}
\date{Modified: August 26, 2019. Compiled: \today}
\maketitle
\tableofcontents
...
Below we show how to generate data in the package \texttt{microbiomeDASim}
with these specifications. First we load the package.
<<pkg_load, warning=FALSE, message=FALSE, eval=FALSE>>=
...
triv_ex <- mvrnorm_sim(n_control=10, n_treat=10, control_mean=2,
                        sigma=1, num_timepoints=2, rho=0.8,
                        corr_str="ar1", func_form="linear",
                        beta= c(0, 1), missing_pct=0,
